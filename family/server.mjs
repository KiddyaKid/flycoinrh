...
try{
 browser=await chromium.launchPersistentContext(resolve(root,'build/external-pilot/browser-profile'),{channel:'chrome',headless:true,viewport:{width:1280,height:720},args:['--remote-debugging-port=5192','--remote-debugging-address=127.0.0.1']});page=await browser.newPage();
 ...
 try{await Promise.all([
  ...
  lane('browser',100,async()=>{if(!readPilotView(root)){if(Date.now()-lastWalletCheck>15000){lastWalletCheck=Date.now();try{const connected=await connectObserverWallet(page);observerWallet={status:connected?'connected-read-only':'disconnected',owner,signing:'paused'};}catch{observerWallet={status:'connection-check-required',owner,signing:'paused'};}}await roam();}},()=>{cameraError='Browser navigation unavailable';}),
  ...
  lane('brain',10000,async()=>{if(!worker)startBrain();if(!worker||brainBusy||!jpg)return;brainBusy=true;try{const hasJob=db.prepare("SELECT count(*) n FROM triggers WHERE status='queued'").get().n||births().some(b=>['eligible','awaiting-signature','submitted'].includes(b.status));if(hasJob&&!observerError&&process.env.FAMILY_OBSERVE_ONLY!=='1')await processEgg();else processing=process.env.FAMILY_OBSERVE_ONLY==='1'?{stage:'paused',detail:'Signing paused; live browser and neural observations continue',asOf:stamp()}:null;if(!neural?.asOf||Date.now()-Date.parse(neural.asOf)>9000)neural={...await think(null),asOf:stamp()};}finally{brainBusy=false;}},e=>{processing={stage:'paused',detail:/^[A-Z_]+$/.test(e.message)?e.message:'MODEL_CHECK_REQUIRED',asOf:stamp()};}),
