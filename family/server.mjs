...
import {launchStatus,readSession,pauseSession} from './lib/launch-session.mjs';
...
async function processEgg(){
 ...
 }catch(e){if(process.env.FAMILY_LAUNCH_SESSION&&['RECONCILE_WALLET_NONCES','INSUFFICIENT_LAUNCH_BALANCE'].includes(e.message)){try{pauseSession(e.message);}catch{}}b.preparationError=/^[A-Z_]+$/.test(e.message)?e.message:'DEPLOYMENT_CHECK_REQUIRED';save(b);}
}
...
try{
 ...
 try{await Promise.all([
  ...
  lane('brain',1000,async()=>{if(!worker)startBrain();if(!worker||brainBusy||!jpg)return;brainBusy=true;try{const hasJob=db.prepare("SELECT count(*) n FROM triggers WHERE status='queued'").get().n||births().some(b=>['eligible','awaiting-signature','submitted'].includes(b.status));if(hasJob&&!observerError&&(!launchStatus().signingPaused||births().some(b=>b.status==='submitted')))await processEgg();else processing=launchStatus().signingPaused?{stage:'paused',detail:launchStatus().reason,asOf:stamp()}:null;const input=dossiers.next();if(input){try{neural={...await think(input.kind,true),asOf:stamp(),tradeId:input.id,tradeHash:input.hash};dossiers.complete(input,neural);}catch(e){dossiers.fail(input);throw e;}}else if(!neural?.asOf||Date.now()-Date.parse(neural.asOf)>9000)neural={...await think(null),asOf:stamp()};}finally{brainBusy=false;}},e=>{processing={stage:'paused',detail:/^[A-Z_]+$/.test(e.message)?e.message:'MODEL_CHECK_REQUIRED',asOf:stamp()};}),
