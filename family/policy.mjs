...
export function publicBirth(b){const {id,status,createdAt,confirmedAt,triggerKind,tradeHash,parents,generation,ordinal,name,symbol,genome,naming,hash,token,receiptVerified,preparationError,test,inputMode,sourceToken}=b;const response=b.readout?{neurons:b.readout.neurons,windowMs:b.readout.windowMs,gain:b.readout.gain,kind:b.readout.kind,parents:b.readout.parents.map(p=>({id:p.id,seed:p.seed,active:p.active,rates:p.rates}))}:null;return {id,status,createdAt,confirmedAt,triggerKind,tradeHash,parents,generation,ordinal,name,symbol,genome,naming,hash,token,receiptVerified,preparationError,test,inputMode,sourceToken,response};}
