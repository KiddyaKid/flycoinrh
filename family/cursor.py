...
pilot=FlyPilot(fb,eye=FlyEye(fb,annotations),sim_steps=100,click_hz=330.,annotations_path=annotations)
for line in sys.stdin:
 try:
  ...
  print(json.dumps(dict(id=c['id'],dx=float(dx),dy=float(dy),click=bool(click),hz=hz,seed=c['seed'],graphSha256=graph_hash,neurons=fb.n,windowMs=20,gain=gain)),flush=True)
