...
export function videoServer(getFrame,port=5191){
 const clients=new Set(),encoders=new Set();let last=null,packet=null,frames=0,bytes=0;
 ...
 const timer=setInterval(()=>{const frame=getFrame();if(!frame)return;const changed=frame!==last;last=frame;
  if(changed){packet=Buffer.concat([Buffer.from('--familyframe\r\nContent-Type: image/jpeg\r\nContent-Length: '+frame.length+'\r\n\r\n'),frame,Buffer.from('\r\n')]);frames++;}
  // FFmpeg needs a clock even when a reading page is completely still. Reuse the
  // already encoded image; direct viewers continue receiving changed frames only.
  for(const res of clients){if((!changed&&!encoders.has(res))||res.writableLength>131072)continue;res.write(packet);bytes+=packet.length;}
 },40);
 const server=createServer((req,res)=>{
  ...
  clients.add(res);
  if(/^Lavf\//.test(req.headers['user-agent']??''))encoders.add(res);
  res.on('close',()=>{clients.delete(res);encoders.delete(res);});
 });
 server.listen(port,'127.0.0.1');
 return {address:()=>server.address(),metrics:()=>({clients:clients.size,limit:8,frames,bytes}),close(){clearInterval(timer);for(const client of clients)client.destroy();server.close();}};
}
