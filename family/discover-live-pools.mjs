...
const current=JSON.parse(readFileSync(new URL('./live-input-pools.json',import.meta.url),'utf8'));
const base=current.curve?current:JSON.parse(readFileSync(new URL('./external-flybrain.json',import.meta.url),'utf8'));
...
try{
 ...
 const logs=ids.length?await rpc.getLogs({address:base.manager,fromBlock:0,toBlock:block.number,topics:[init.getEvent('Initialize').topicHash,ids]}):[];
 ...
 const config={...(current.curve?current:{}),chainId:4663,token:base.token,verifiedAt:new Date().toISOString(),verifiedBlock:block.number,coverage:current.curve?'Official PONS curve, canonical graduation pool and discovered V4 pools':'Discovered Uniswap V4 pools; excludes V3 and newly created pools until rediscovery',pools:[...new Map([...(current.curve?current.pools:[]),...pools].map(p=>[p.poolId,p])).values()]};
